...
const MetricCard = ({ label, value, sub, Icon, accentColor, isCurrency, isPercent, isLoading, isPositive = true, alertMode = false }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl bg-white border shadow-2xs hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col justify-between gap-3 group ${alertMode ? 'border-amber-300 bg-amber-50/20' : 'border-slate-200/90 hover:border-slate-300'
      ...
      <div className="min-w-0">
        {isLoading ? (
          <div className="skeleton h-7 w-24 my-0.5" />
        ) : (
          <p
            className="text-xl sm:text-2xl font-bold font-mono tracking-tight text-slate-900 tabular-nums animate-count truncate"
            title={display}
          >
            {display}
          </p>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      <MetricCard
        ...
        sub={`${s.status_breakdown?.recovered || 0} rescued`}
      ...
      <MetricCard
        ...
        sub={`${s.ptp_committed_count || 0} scheduled`}
        Icon={IconCalendar}
        accentColor="#7c3aed"
        isCurrency
        isLoading={isLoading}
      ...
      <MetricCard
        ...
        sub={`${s.total_transactions || 0} ledger items`}
      ...
      <MetricCard
        ...
        sub={exceptionTotal > 0 ? "manual review queue" : "all queues clear"}
        Icon={IconXCircle}
        accentColor={exceptionTotal > 0 ? "#dc2626" : "#64748b"}
        alertMode={exceptionTotal > 0}
        isLoading={isLoading}
      />
    </div>
  );
